...
@torch.no_grad()
def get_nmi_ari(z, y, c, rs=0):
    pred_y = KMeans(n_clusters=c, random_state=rs).fit(z.detach().cpu().numpy()).labels_
    nmi = normalized_mutual_info_score(y, pred_y)
    ari = adjusted_rand_score(y, pred_y)
    return nmi, ari